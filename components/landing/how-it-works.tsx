      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-20">
          ...
          <p className="text-zinc-400 text-lg max-w-xl mx-auto">
            No complex setup. No configuration files. Just effective code
            reviews.
          </p>
        </div>
          ...
          {steps.map((step, index) => (
            <div key={step.id} className="relative z-10 group">
              ...
              >
                {/* Icon Container */}
                <div
                  className={`w-20 h-20 rounded-2xl bg-gradient-to-br ${step.gradient} p-[1px] mb-8 relative shadow-xl shadow-black/50 group-hover:-translate-y-2 transition-transform duration-300`}
                >
                  <div className="w-full h-full bg-[#0F1117] rounded-2xl flex items-center justify-center relative overflow-hidden">
                    <div
                      className={`absolute inset-0 bg-gradient-to-br ${step.gradient} opacity-10 group-hover:opacity-20 transition-opacity`}
                    />
                    {step.icon}
                  </div>
