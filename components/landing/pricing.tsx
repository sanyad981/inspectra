...
function PricingCard({
  ...
  return (
    <div
      className={`relative p-8 rounded-3xl border ${highlight ? "border-purple-500 bg-purple-500/5" : "border-white/10 bg-white/5"} flex flex-col`}
    >
      {highlight && (
      ...
      <ul className="space-y-4 mb-8 flex-1">
        {features.map((feature, i) => (
          <li key={i} className="flex items-center gap-3 text-sm text-zinc-300">
            <Check
              className={`w-4 h-4 ${highlight ? "text-purple-400" : "text-zinc-500"}`}
            />
            {feature}
          </li>
        ))}
      ...
      <Button
        className={`w-full h-12 rounded-xl text-base font-medium ${highlight ? "bg-gradient-to-r from-purple-600 to-pink-600 hover:opacity-90 text-white" : "bg-white text-black hover:bg-zinc-200"}`}
      >
        Get Started
      </Button>
    </div>
