  ...
  return (
    <group ref={group}>
        {Array.from({ length: 15 }).map((_, i) => (
             <mesh key={i} position={[ (Math.random() - 0.5) * 50, (Math.random() - 0.5) * 50, (Math.random() - 0.5) * 20 ]} rotation={[Math.random() * Math.PI, Math.random() * Math.PI, 0]}>
                <cylinderGeometry args={[0.02, 0.02, 20]} />
                <meshBasicMaterial color={new THREE.Color().setHSL(Math.random(), 0.8, 0.5)} transparent opacity={0.1} blending={THREE.AdditiveBlending} depthWrite={false} />
             </mesh>
        ))}
    </group>
  )
  ...
  return (
    <div className="fixed inset-0 -z-10 h-screen w-full pointer-events-none">
      <Canvas camera={{ position: [0, 0, 10], fov: 60 }} gl={{ alpha: true, antialias: true }}>
        ...
        <Sparkles count={500} scale={20} size={2} speed={0.4} opacity={0.5} color="#8b5cf6" />
        <Sparkles count={300} scale={25} size={3} speed={0.3} opacity={0.4} color="#00b4d8" />
