...
export default function GithubInlineComments({
  ...
  const [comments, setComments] = React.useState<{ [key: number]: any[] }>(
    initialComments,
  );
                ...
                <React.Fragment key={index}>
                  ...
                  >
                    ...
                    {line.kind === "hunk" ? (
                      <td
                        colSpan={3}
                        className="px-4 py-2 text-center text-xs bg-[#161b22] border-b border-zinc-800/50"
                      >
                        {line.content}
                      </td>
                    ) : (
                      <>
                        ...
                        <td className="w-[1%] min-w-[50px] px-2 py-1 text-right text-zinc-600 select-none border-r border-zinc-800/50 relative">
                          ...
                          {(isHovered || activeCommentLine === index) &&
                            line.kind !== "del" && (
                              <button
                              ...
                              <div className="rounded-lg border border-zinc-700 bg-[#161b22] shadow-xl overflow-hidden">
                                ...
                                {comments[index]?.map(
                                  (comment: any, i: number) => (
                                    <div
                                      key={i}
                                      className="border-b border-zinc-800 last:border-0 relative"
                                    >
                                      <div
                                        className={cn(
                                      ...
                                      >
                                        ...
                                        {comment.isAi && (
                                          <span className="ml-auto text-[10px] uppercase tracking-wider font-bold text-blue-500 bg-blue-500/10 px-1.5 py-0.5 rounded">
                                            AI
                                          </span>
                                        )}
                                      </div>
