        ...
        <Sidebar>
            <SidebarContent>
                <SidebarHeader>
                    ...
                    <SidebarContent>
                        <div className='m-2'>
                            <p className='text-xs font-semibold text-sidebar-foreground tracking-wide'>MENU</p>
                        </div>
                    </SidebarContent>
                    <SidebarMenu  >
                        {navigationItems.map((item) => (
                            <SidebarMenuItem key={item.href} className='p-1' >
                                <SidebarMenuButton asChild isActive={isActive(item.href)} tooltip={item.label} className={`h-10 px-4 rounded-lg transition-colors duration-200 ${isActive(item.href) ? "bg-sidebar-accent text-sidebar-accent-foreground font-semibold" : "hover:bg-sidebar-accent/50 text-sidebar-foreground"}`}>
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <SidebarMenuButton >
                                    ...
                                    <div>
                                        <p className="text-xs font-semibold text-sidebar-foreground tracking-wide">{name}</p>
                                        <p className="text-xs font-medium text-sidebar-foreground/90">{email}</p>
                                    </div>
                            ...
                            <DropdownMenuContent className='w-56 rounded-lg ' side="top" align="start" sideOffset={4}>
                                <DropdownMenuItem onClick={() => setTheme(theme === "dark" ? "light" : "dark")} className='py-2'>
